...
/**
 * @title Gefion Tokenized Strategy
...
contract TokenizedStrategy {
    ...
    struct StrategyData {
        ...
        // I know Gefion moves slowly but surely V4 will be out by then.
    ...
    bytes32 internal constant BASE_STRATEGY_STORAGE =
        bytes32(uint256(keccak256("gefion.base.strategy.storage")) - 1);
     ...
     * @dev Will be 'gs + asset symbol'.
     * @return . The symbol the strategy is using for its tokens.
     */
    function symbol() external view returns (string memory) {
        return
            string(abi.encodePacked("gs", _strategyStorage().asset.symbol()));
    }
